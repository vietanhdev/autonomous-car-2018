...
DetectedObject::DetectedObject(int label, cv::Rect position) {
    this->label = label;
    ...
    if ((dir = opendir(obstacles_folder_path.c_str())) != NULL) {
        ...
        while ((ent = readdir(dir)) != NULL) {
            if (strcmp(ent->d_name, ".") != 0 &&
                ...
                std::string file_ext = ObstacleDetector::getFileExt(file_path);
                std::transform(file_ext.begin(), file_ext.end(),
                               file_ext.begin(), ::tolower);
                if (file_ext == "jpg" || file_ext == "png" ||
                    file_ext == "bmp") {
                    std::cout << "Loading Obstacle: " << file_path << std::endl;
...
void ObstacleDetector::updateDetectedList(std::vector<DetectedObject> &objects,
        ...
        for (int j = 0; j < new_object_labels.size(); ++j) {
            if (!processed[j]) {
                ...
                if (objects[i].label == new_object_labels[j] &&
                    (objects[i].last_position & new_object_pos[j]).area() > 0) {
...
int ObstacleDetector::detect(const cv::Mat &img,
        ...
        for (int i = 0; i < detected_objects.size(); ++i) {
            if (detected_objects[i].disappeared_times < 2 &&
                ...
                cv::putText(draw, std::to_string(detected_objects[i].label),
                            ...
                            FONT_HERSHEY_COMPLEX, 2, Scalar(255, 255, 255), 1);
            }
        }
...
bool ObstacleDetector::matching(const Mat &img, const Mat &templ, std::vector<cv::Rect> &rects) {
    // Clear the result
    rects.clear();
...
std::string ObstacleDetector::getFileExt(const std::string &s) {
